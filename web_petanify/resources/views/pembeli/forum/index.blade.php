<x-app-layout>
    ...
    <div class="py-6 forum-wrapper">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 forum-main">
            ...
            <!-- Posting Form -->
            <form method="POST" action="{{ route('pembeli.post.store') }}" enctype="multipart/form-data" class="forum-post-form">
                @csrf
            ...
            @foreach($posts as $post)
                <div class="forum-post-card">
                    <div class="forum-post-header">
                        {{-- FOTO PROFIL ASLI PEMBUAT POST --}}
                        <img src="{{ $post->user->foto ? asset('storage/foto_profil/'.$post->user->foto) : asset('img/default-user.png') }}"
                             alt="{{ $post->user->name }}" class="forum-avatar" />
                        <div>
                            <b>{{ $post->user->name }}</b>
                    ...
                    <div class="forum-comments">
                        @foreach($post->comments as $comment)
                            <div class="forum-comment">
                                {{-- FOTO PROFIL ASLI KOMENTATOR --}}
                                <img src="{{ $comment->user->foto ? asset('storage/foto_profil/'.$comment->user->foto) : asset('img/default-user.png') }}"
                                     alt="{{ $comment->user->name }}" class="forum-avatar-small" />
                        ...
                        <form method="POST" action="{{ route('pembeli.comment.store', $post->id) }}" class="forum-comment-form">
                            @csrf
