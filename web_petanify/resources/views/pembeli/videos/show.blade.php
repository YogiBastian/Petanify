    ...
    <section class="flex flex-col md:flex-row gap-8 py-10 bg-gray-50 min-h-[90vh]">
        ...
        <div class="flex-1 max-w-3xl mx-auto bg-white rounded-2xl shadow-xl border p-0 overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-white">
                ...
                >
                <div>
                    <div class="font-semibold text-gray-700">Admin Petanify</div>
                    <div class="text-gray-400 text-xs">
                        {{ $video->published_at ? $video->published_at->diffForHumans() : 'Baru diupload' }}
                    </div>
                </div>
                <div class="ml-auto flex gap-2">
                    <button class="flex items-center gap-1 px-4 py-2 rounded-full bg-gray-100 text-gray-700 hover:bg-green-100 hover:text-green-800 font-semibold text-sm transition">
                        <i class="fas fa-share"></i> Share
                    </button>
                    {{-- LIKE BUTTON --}}
                    <form method="POST" action="#">
                        @csrf
                        <button type="button" class="flex items-center gap-1 px-4 py-2 rounded-full 
                            {{ $video->isLikedBy(auth()->user()) ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-green-600 hover:text-white' }} 
                            font-semibold text-sm transition">
                            <i class="fas fa-heart"></i> 
                            {{ $video->isLikedBy(auth()->user()) ? 'Liked' : 'Like' }}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="px-6 py-6 bg-white">
                ...
                <div class="flex gap-4 text-gray-500 text-xs mb-4">
                    <div><i class="fas fa-eye"></i> {{ number_format($video->views ?? 0) }} views</div>
                    <div><i class="fas fa-thumbs-up"></i> {{ $video->likeCount() }} likes</div>
                    <div><i class="fas fa-clock"></i> {{ $video->published_at ? $video->published_at->diffForHumans() : 'Baru diupload' }}</div>
                </div>
                {{-- Komentar --}}
                <div class="mt-8">
                    <h3 class="text-lg font-bold text-gray-800 mb-3">Komentar</h3>
                    <form action="{{ route('pembeli.videos.komentar.store', $video->id) }}" method="POST" class="flex gap-2 mb-4">
                        @csrf
                        ...
                        @foreach($video->comments as $comment)
                            <div class="bg-gray-100 p-3 rounded-lg flex gap-2 items-start">
                                ...
                                <div>
                                    ...
                                    <div class="text-gray-600 text-sm">{{ $comment->comment }}</div>
                ...
                <a href="{{ route('pembeli.videos.index') }}"
            ...
            <div class="bg-white rounded-2xl shadow border p-4">
                ...
                <div class="space-y-3">
                    @foreach($relatedVideos as $related)
                        <a href="{{ route('pembeli.videos.show', $related->id) }}" class="flex gap-3 hover:bg-gray-100 p-2 rounded-lg transition">
