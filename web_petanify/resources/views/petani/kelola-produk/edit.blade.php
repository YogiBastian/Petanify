<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto mt-10">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
            <form action="{{ route('petani.kelola-produk.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label class="block font-semibold mb-1">Nama Produk <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_produk" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" required value="{{ old('nama_produk', $product->nama_produk) }}">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="harga" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" required value="{{ old('harga', $product->harga) }}">
                    ...
                    <div>
                        ...
                        <input type="number" name="stok" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" required min="0" value="{{ old('stok', $product->stok) }}">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="satuan" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" maxlength="20" value="{{ old('satuan', $product->satuan) }}" placeholder="Contoh: kg, liter, dus">
                        @error('satuan') <small class="text-red-500">{{ $message }}</small>@enderror
                    </div>
                    <div>
                        <label class="block font-semibold mb-1">Diskon (Rp)</label>
                        <input type="number" name="diskon" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" min="0" value="{{ old('diskon', $product->diskon) }}">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none">{{ old('deskripsi', $product->deskripsi) }}</textarea>
                ...
                <div>
                    ...
                    <select name="kategori_id" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-200 focus:outline-none" required>
                        ...
                        @foreach ($kategoris as $id => $nama)
                            <option value="{{ $id }}" {{ old('kategori_id', $product->kategori_id) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block font-semibold mb-1">Foto Produk</label><br>
                    @if ($product->foto)
                        <img src="{{ asset('storage/'.$product->foto) }}" class="w-28 mb-2 rounded shadow">
                    @endif
